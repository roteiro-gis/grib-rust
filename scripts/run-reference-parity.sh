...
docker run --rm \
  ...
  GRIB_READER_ECCODES_HELPER=/workspace/target/eccodes-reference /usr/local/cargo/bin/cargo test -p grib-reader --test parity_eccodes --all-features -- --ignored
  GRIB_READER_ECCODES_HELPER=/workspace/target/eccodes-reference /usr/local/cargo/bin/cargo test -p grib-writer --test parity_eccodes --all-features -- --ignored
'
